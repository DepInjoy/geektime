数据库集簇(database cluster)是指由单个PostgreSQL服务器实例管理的数据库(database)集合，一个 PostgreSQL服务器只会在单机上运行并管理单个数据库集簇。数据库集簇在本质上就是一个文件目录，其包含着一些列子目录与文件。数据库是数据库对象(database object)的集合。
