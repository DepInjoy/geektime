...
如果在一个异步提交和写事务的WAL记录中间时发生数据库崩溃， 在事务期间的修改***将\***会丢失。风险的持续时间会被限制，风险的实际最大持续时间是三倍的`wal_writer_delay`(`WAL writer`后台进程会每隔`wal_writer_delay`毫秒就向磁盘刷入未写入的WAL记录)， 因为WAL写进程被设计为支持在繁忙时一次写入所有块。
