...
- 集合操作，其中有UNION(并) $R \cup S$; Intersection(交) $R \cap S $ ; Difference(差) $R - S$; Cartesian Product(笛卡尔积) $R \times S$
- 纯关系操作，其中有Project(投影) $\pi_A(R)$;  Select(选择) $\sigma_{con}(R)$; Join(连接) $R \Join_{A \theta B} S$; Division(除) $R \div S $
